...
public class PlayerController : MonoBehaviour
{
    ...
    private Rigidbody2D rd; // コンポーネントの取得用
    ...
    private Vector2 lookDirection = new Vector2(0f, -1.0f);  // キャラの向きの情報の設定用

    // Start is called before the first frame update
    void Start()
    {
        // このスクリプトがアタッチされているゲームオブジェクトにアタッチされているコンポーネントの中から、<指定>したコンポーネントの情報を取得して、左辺に用意した変数に代入
        rd = GetComponent<Rigidbody2D>(); // あるいは、TryGeyComponent(out rb);　でも可
